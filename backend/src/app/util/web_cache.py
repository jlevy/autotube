...
class DirStore:
    """A simple file storage scheme: A directory of items, organized into
    folders, stored by readable but uniquely hashed keys so it's possible to
...
class WebCache(DirStore):
    """
    The web cache is a DirStore with a fetching mechanism based on a fixed
    object expiration time.

    Fetch timestamp is modification time on file. Thread safe since file
    creation is atomic.
    """
